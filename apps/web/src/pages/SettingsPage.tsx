import type { NotificationLocale, Role } from "@minecontrol/shared";
import { ROLES } from "@minecontrol/shared";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useEffect, useState, type FormEvent } from "react";
import { useTranslation } from "react-i18next";
import { useAuth } from "../auth/AuthContext.js";
import { LANGUAGE_NAMES, SUPPORTED_LANGUAGES } from "../i18n/index.js";
  ...
  return (
    <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
      <div className="mb-1 flex items-center gap-2">
        <h2 className="font-semibold">{t("twoFactor.title")}</h2>
        {status?.enabled && (
      ...
      <p className="mb-4 text-sm text-neutral-500">{t("twoFactor.description")}</p>

      {status?.enabled ? (
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-sm">
            <span className="mb-1 block text-neutral-400">{t("twoFactor.disableCodeLabel")}</span>
            {codeInput}
          </label>
          <button
        ...
        <div className="space-y-3">
          <p className="text-sm text-neutral-300">{t("twoFactor.scanHint")}</p>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="text-xs text-neutral-500">
              <p className="mb-1">{t("twoFactor.manual")}</p>
          ...
          <div className="flex flex-wrap items-end gap-2">
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("twoFactor.confirmCodeLabel")}</span>
              {codeInput}
            </label>
            <button
        ...
        <button
          ...
          className="rounded-md bg-status-online px-4 py-2 text-sm font-medium text-neutral-950 hover:opacity-90 disabled:opacity-50"
        >
          {setupMutation.isPending ? "…" : t("twoFactor.setup")}
        </button>
      )}
    </section>
  );
...
function ApiTokenSettings() {
  ...
  return (
    <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
      <h2 className="mb-1 font-semibold">{t("tokens.title")}</h2>
      ...
      {created && (
        <div className="mb-4 rounded-md border border-status-online/40 bg-status-online/5 p-3">
          <p className="mb-1 text-xs text-neutral-400">{t("tokens.newTokenHint")}</p>
          ...
          {tokens.map((token) => (
            <li key={token.id} className="flex items-center justify-between gap-3 py-2 text-sm">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="truncate font-medium text-neutral-200">{token.name}</span>
                  <span className="rounded bg-neutral-800 px-1.5 py-0.5 text-xs text-neutral-400">
                    {token.role}
                  </span>
                  <code className="text-xs text-neutral-600">{token.prefix}…</code>
                </div>
                <div className="text-xs text-neutral-500">
                  {token.lastUsedAt
                    ? t("tokens.lastUsed", { time: formatRelative(token.lastUsedAt) })
                    : t("tokens.neverUsed")}
                  {token.expiresAt
                    ? ` · ${t("tokens.expires", { date: formatDateTime(token.expiresAt) })}`
                    : ""}
                </div>
              </div>
              <button
                onClick={() =>
                  void confirmDialog({
                    ...
                    message: t("tokens.revokeMessage", { name: token.name }),
                    confirmLabel: t("tokens.revoke"),
                    danger: true,
                  }).then((ok) => ok && revokeMutation.mutate(token.id))
                }
      ...
      >
        ...
        <label className="text-sm">
          <span className="mb-1 block text-neutral-400">{t("common:labels.role")}</span>
          <select
            value={role}
        ...
        <label className="text-sm">
          <span className="mb-1 block text-neutral-400">{t("tokens.expiryDays")}</span>
          <input
            type="number"
            min={1}
            value={expires}
        ...
        <button
          ...
          className="rounded-md bg-status-online px-4 py-2 text-sm font-medium text-neutral-950 hover:opacity-90 disabled:opacity-50"
        >
          {createMutation.isPending ? t("common:actions.creating") : t("tokens.create")}
        </button>
      </form>
    </section>
  );
...
function NotificationSettings() {
  ...
  const [notifyLocale, setNotifyLocale] = useState<NotificationLocale>("de");
  ...
  useEffect(() => {
    if (data) {
      ...
      setNotifyLocale(data.notifyLocale);
    }
  }, [data]);
  ...
  return (
    <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
      <h2 className="mb-1 font-semibold">{t("notifications.title")}</h2>
      <p className="mb-4 text-sm text-neutral-500">{t("notifications.description")}</p>

      <form onSubmit={onSubmit} className="space-y-5">
        <div>
          <h3 className="mb-2 text-sm font-medium text-neutral-300">
            {t("notifications.discordWebhook")}
          </h3>
          <label className="mb-1 block text-sm text-neutral-400">
            {t("notifications.webhookUrl")}{" "}
            {discordConfigured && !clearWebhook && (
              <span className="text-xs text-status-online">{t("notifications.configured")}</span>
            )}
          </label>
          <input
            type="password"
            ...
            placeholder={
              discordConfigured
                ? t("notifications.unchangedPlaceholder")
        ...
        <div className="border-t border-neutral-800 pt-4">
          <div className="mb-2 flex items-center gap-2">
            <h3 className="text-sm font-medium text-neutral-300">{t("notifications.emailSmtp")}</h3>
          ...
          <div className={`grid gap-2 sm:grid-cols-2 ${clearEmail ? "opacity-50" : ""}`}>
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.smtpHost")}</span>
              <input
                value={smtpHost}
                disabled={clearEmail}
            ...
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.port")}</span>
              <input
                type="number"
                min={1}
                max={65535}
            ...
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.username")}</span>
              <input
                value={smtpUser}
                disabled={clearEmail}
            ...
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.password")}</span>
              <input
                type="password"
                value={smtpPassword}
                disabled={clearEmail}
                onChange={(e) => setSmtpPassword(e.target.value)}
                placeholder={emailConfigured ? t("notifications.unchangedPlaceholder") : ""}
                className={inputClass}
              />
            </label>
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.from")}</span>
              <input
                type="email"
                value={emailFrom}
                disabled={clearEmail}
            ...
            <label className="text-sm">
              <span className="mb-1 block text-neutral-400">{t("notifications.to")}</span>
              <input
                value={emailTo}
                disabled={clearEmail}
                onChange={(e) => setEmailTo(e.target.value)}
                placeholder={t("notifications.toPlaceholder")}
                className={inputClass}
              />
            </label>
          </div>
        ...
        <fieldset className="space-y-2 border-t border-neutral-800 pt-4">
          <legend className="mb-1 text-sm text-neutral-400">
            {t("notifications.eventsLegend")}
          </legend>
          <Toggle
            label={t("notifications.eventServerDown")}
            checked={serverDown}
            onChange={setServerDown}
          />
          <Toggle
            label={t("notifications.eventBackupFailed")}
            checked={backupFailed}
            onChange={setBackupFailed}
          />
          <Toggle
            label={t("notifications.eventTaskFailed")}
            checked={taskFailed}
        ...
        <fieldset className="space-y-2 border-t border-neutral-800 pt-4">
          <legend className="mb-1 text-sm text-neutral-400">
            {t("notifications.languageLegend")}
          </legend>
          <p className="text-xs text-neutral-500">{t("notifications.languageHint")}</p>
          <select
            value={notifyLocale}
            onChange={(e) => setNotifyLocale(e.target.value as NotificationLocale)}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          ...
          >
            {t("notifications.sendDiscordTest")}
          </button>
          <button
            type="button"
            ...
            className="rounded-md border border-neutral-700 px-4 py-2 text-sm hover:bg-neutral-800 disabled:opacity-50"
          >
            {t("notifications.sendEmailTest")}
          </button>
          {saveMutation.isSuccess && (
            <span className="text-sm text-status-online">{t("notifications.saved")}</span>
          )}
          ...
          {discordTestMutation.isSuccess && (
            <span className="text-sm text-status-online">{t("notifications.sentDiscord")}</span>
          )}
          ...
          {emailTestMutation.isSuccess && (
            <span className="text-sm text-status-online">{t("notifications.sentEmail")}</span>
          )}
